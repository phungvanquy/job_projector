...
<p style="text-align:justify" >1. Из ПК (master) Modbus Request запись в holding register (1001) отправлена на Arduino (slave). В этом Request включается значение Angle_value для управления прожектором. Arduino получает эти значений и управляет мотором так чтобы прожектор поворачивал к желаемому углу. Одновременно с этим master может читать значние текущего угла поворачивания прожектора с момощью modbus Request на читать holding регистор (1002) </p>
